...
import com.hos.sys.modular.system.mapper.SysCategoryMapper;
	...
	public List<TreeSelectModel> queryListByCode(String pcode) throws ServiceException {
		...
		if(oConvertUtils.isNotEmpty(pcode)) {
			...
			if(list==null || list.size() ==0) {
				throw new ServiceException(4584,"该编码【"+pcode+"】不存在，请核实!");
			}
			if(list.size()>1) {
				throw new ServiceException(4584,"该编码【"+pcode+"】存在多个，请核实!");
			}
