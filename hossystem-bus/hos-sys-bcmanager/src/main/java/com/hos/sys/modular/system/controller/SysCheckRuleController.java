...
public class SysCheckRuleController extends JeecgController<SysCheckRule, ISysCheckRuleService> {
    ...
    public ResponseData checkByCode(
        ...
        if (sysCheckRule == null) {
            return ResponseData.error("该编码不存在");
        }
        JSONObject errorResult = sysCheckRuleService.checkValue(sysCheckRule, URLDecoder.decode(value, "UTF-8"));
        if (errorResult == null) {
            return ResponseData.success();
        } else {
            return ResponseData.error(ResponseData.DEFAULT_ERROR_CODE,"校验不通过!",errorResult);
        }
    }
    ...
    public ResponseData add(@RequestBody SysCheckRule sysCheckRule) {
    ...
    public ResponseData edit(@RequestBody SysCheckRule sysCheckRule) {
    ...
    public ResponseData queryById(@RequestParam(name = "id", required = true) String id) {
        ...
        return ResponseData.success(sysCheckRule);
    }
