...
import com.hos.base.consts.CommonSendStatus;
...
import com.hos.sys.modular.system.service.ISysAnnouncementService;
import com.hos.sys.modular.system.vo.LoginUser;
import com.hos.sys.modular.websocket.WebSocket;
...
public class SysAnnouncementController {
	...
	public ResponseData queryPageList(SysAnnouncement sysAnnouncement,
	...
	public ResponseData add(@RequestBody SysAnnouncement sysAnnouncement) {
		ResponseData result = new ResponseData();
		try {
			sysAnnouncement.setDelFlag(CommonConstant.DEL_FLAG_0.toString());
	...
	public ResponseData eidt(@RequestBody SysAnnouncement sysAnnouncement) {
	...
	public ResponseData queryById(@RequestParam(name="id",required=true) String id) {
		...
		if(sysAnnouncement==null) {
			...
			return ResponseData.success(sysAnnouncement);
		}
	}
	...
	public ResponseData doReleaseData(@RequestParam(name="id",required=true) String id, HttpServletRequest request) {
		SysAnnouncement sysAnnouncement = sysAnnouncementService.getById(id);
		if(sysAnnouncement==null) {
			return ResponseData.error("未找到对应实体");
		}else {
			...
			if(ok) {
				...
				return ResponseData.success("该系统通知发布成功");
			}
			return ResponseData.error("该系统通知发布失败!");
		}
	}
	...
	public ResponseData doReovkeData(@RequestParam(name="id",required=true) String id, HttpServletRequest request) {
		SysAnnouncement sysAnnouncement = sysAnnouncementService.getById(id);
		if(sysAnnouncement==null) {
			return ResponseData.error("未找到对应实体");
		}else {
			...
			boolean ok = sysAnnouncementService.updateById(sysAnnouncement);
			if(ok) {
				return ResponseData.success("该系统通知撤销成功!");
			}
			return ResponseData.error("该系统通知撤销失败!");
		}
	}
...
//		LoginUser sysUser = (LoginUser)SecurityUtils.getSubject().getPrincipal();
		...
		return ResponseData.success(sysMsgMap);
	}
    ...
    public ResponseData importExcel(HttpServletRequest request, HttpServletResponse response) {
        ...
        for (Map.Entry<String, MultipartFile> entity : fileMap.entrySet()) {
            ...
            try {
				...
				return ResponseData.success("文件导入成功！数据行数：" + listSysAnnouncements.size());
			} catch (Exception e) {
				log.error(e.getMessage(),e);
